...
function App() {
    ...
    useEffect(() => {
        const initializeApp = async () => {
            ...
            // Получаем данные пользователя Telegram или создаем тестовые данные в режиме разработки
            let tgUser;
            ...
            // Если мы в режиме разработки и данные не получены, создаем тестового пользователя
            if (isDevMode && !tgUser) {
                tgUser = {
                    id: 123456789,
                    first_name: "Тестовый",
    ...
    useEffect(() => {
        // Тестирование Firebase только в режиме разработки
        ...
        // Запускаем тесты только в режиме разработки и когда получили данные пользователя
    ...
    const handleRegistration = async (userData) => {
        try {
            // Проверяем аутентификацию перед отправкой данных
            if (!auth.currentUser) {
                await signInAnonymouslyIfNeeded();
                // Если здесь все еще нет пользователя, то выбрасываем ошибку
                if (!auth.currentUser) {
