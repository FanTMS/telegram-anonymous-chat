...
function App() {
    ...
    const handleRegistration = async (userData) => {fNeeded();
        try {
            if (!auth.currentUser) {onsole.warn("Аутентификация не выполнена, но продолжаем работу:", authError);
...
= await addDoc(collection(db, "users"), newUser);
            ...
            try {
                WebApp.showPopup({     title: 'Успешная регистрация',
            ...
            } catch (e) {                    console.log("Успешная регистрация! (Уведомление недоступно в режиме разработки)");
...
роверьте правила безопасности Firebase.");
            try {
                WebApp.showPopup({ throw dbError;
                    title: 'Ошибка',
