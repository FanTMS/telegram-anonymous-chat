...
function App() {
    ...
    useEffect(() => {
        const init = async () => {
            try {
                const initialized = await initializeFirebaseCollections();
                setFirebaseInitialized(initialized);
                console.log('Статус инициализации Firebase:', initialized);
            } catch (error) {
    ...
    const handleRegistrationSubmit = async (formData) => {
        try {
            ...
            // Создаем пользователя в Firestore с явным указанием ID документа
            try {
                const usersRef = collection(db, "users");

                // Используем telegramId как ключ документа, если он есть, иначе генерируем ID
                const userId = userDataWithTelegramId.telegramId || `user_${Date.now()}`;
                const userDocRef = doc(usersRef, userId);

                // Устанавливаем данные пользователя с serverTimestamp
                ...
                throw new Error(`Ошибка при создании пользователя: ${firebaseError.code} - ${firebaseError.message}`);
            }
