...
import { testFirebaseConnection, _diagnoseFirebaseIssues as diagnoseFirebaseIssues } from './utils/firebaseDebugUtils';
import { ensureCollectionsExist } from './utils/databaseSetup';
import { isBrowser } from './utils/browserUtils';
import WebApp from '@twa-dev/sdk';
import { getFirestore, _collection as collection, doc, setDoc, _getDoc as getDoc, serverTimestamp } from 'firebase/firestore';
...
function App() {
    ...
    // Эффект для проверки сохраненных данных пользователя
    useEffect(() => {
        const checkSavedUser = async () => {
            setLoading(true);

            try {
                // Пытаемся получить сохраненного пользователя
                ...
                if (savedUserId && savedUserData) {
                    try {
                        ...
                        console.log("Восстановление сессии пользователя:", parsedUser);
                        setUser(parsedUser);
                    } catch (error) {
    ...
    // Сохраняем пользователя в localStorage при любом изменении
    useEffect(() => {
        if (user) {
