...
const AppLayout = () => {
    ...
    const isAuthenticated = localStorage.getItem('registeredUser') !== null;
    ...
    useEffect(() => {
        ...
        if (location.pathname === '/' || location.pathname === '/home') {
            ...
            // Устанавливаем метку, чтобы избежать бесконечных обновлений
            if (!sessionStorage.getItem('guide_visited')) {
                sessionStorage.setItem('guide_visited', 'true');
            }
        }
    ...
    // Обработка перезагрузки руководства
    useEffect(() => {
        if (location.pathname === '/guide') {
            console.log('Инициализация страницы руководства...');
            // Небольшая задержка, чтобы гарантировать загрузку компонента
            const timer = setTimeout(() => {
                // Ничего не делаем, просто убеждаемся, что компонент обновился
            }, 100);
