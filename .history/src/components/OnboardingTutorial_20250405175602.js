...
const OnboardingTutorial = ({ onComplete }) => {
    ...
    return (
        <div className={`${containerClasses} ${isDarkMode ? 'dark-mode' : ''}`} data-step={activeStep} key={animationKey}>
            <button className="theme-toggle" onClick={toggleDarkMode} title={isDarkMode ? "Переключить на светлую тему" : "Переключить на темную тему"}>
                {isDarkMode ? "☀️" : "🌙"}
            </button>
...
export default OnboardingTutorial;};    );        </div>)}                </div>                    </div >                        </button > ОК > onClick={ handleAlertClose } className = "tg-haptic-btn" < button < p > { alertMessage }</p > <div className="alert-content">                <div className="alert">            {showAlert && (<div className="safe-area-bottom"></div>            {/* SafeArea для устройств с "челками" и закругленными углами */}            )}                </div>                    </button>                        Назад        </div >
    );
};
