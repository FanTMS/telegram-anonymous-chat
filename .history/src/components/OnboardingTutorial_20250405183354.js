...
const OnboardingTutorial = ({ onComplete }) => {
    ...
    return (
        <div className={`tutorial-container ${isDarkMode ? 'dark-mode' : ''}`} data-step={activeStep} key={animationKey}>
            ...
            {/* Всегда показываем кнопки навигации, независимо от платформы */}
            ...
            <button
                ...
                onClick={() => {
                    // Добавляем проверку поддержки метода перед вызовом
                    try {
                        if (WebApp && typeof WebApp.switchInlineQuery === 'function') {
                            WebApp.switchInlineQuery('', ['users']);
                        } else {
                            console.warn('WebApp.switchInlineQuery is not supported in this environment');
