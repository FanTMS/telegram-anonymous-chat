...
const RegistrationForm = ({ onSubmit, telegramUser, isDevelopment = false }) => {onForm = ({ onSubmit, telegramUser, isDevelopment = false }) => {
    const [formData, setFormData] = useState({etFormData] = useState({
        name: '',
        age: '', age: '',
        interests: '',        interests: '',
        aboutMe: ''
    ...
    const handleChange = (e) => { (e) => {
        const { name, value } = e.target;ue } = e.target;
    ...
    const validateStep = (step) => {= (step) => {
        ...
        switch (step) {{
            ...
            case 1: // Возраст
                ...
                } else if (parseInt(formData.age) < 17) { if (parseInt(formData.age) < 17) {
            ...
            case 2: // Интересы
                ...
                } else if (formData.interests.length < 5) { if (formData.interests.length < 5) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!userData.name.trim()) {            if (!userData.name.trim()) {
        ...
        return (
            <div className="registration-error">on-error">
                ...
                <buttonbutton
                    ...
                    onClick={() => setErrors({ ...errors, submit: null })}ck={() => setErrors({ ...errors, submit: null })}
                >
