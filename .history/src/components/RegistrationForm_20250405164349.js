...
const RegistrationForm = ({ onSubmit, telegramUser = null, isDevelopment = false }) => {
    ...
    const enhancedHandleSubmit = useCallback(async () => {
        try {
            ...
            try {
                if (typeof WebApp.HapticFeedback !== 'undefined' && WebApp.HapticFeedback.notificationOccurred) {
                    WebApp.HapticFeedback.notificationOccurred('success');
                }

                if (typeof WebApp.showPopup !== 'undefined') {
                    WebApp.showPopup({
    ...
    useEffect(() => {
        ...
        try {
            if (isLastStep) {
                ...
                WebApp.MainButton.onClick(enhancedHandleSubmit);
            } else {
            ...
            return () => {
                if (isLastStep) {
                    WebApp.MainButton.offClick(enhancedHandleSubmit);
                } else {
    ...
    }, [currentStep, isDevelopment, isLastStep, enhancedHandleSubmit, validateStep]);
    ...
    return (
        <div className="registration-container">
            ...
            {/* Фиксированная кнопка для завершения регистрации, всегда видна на третьем шаге */}
            {currentStep === 2 && (
                <button
                    type="button"
