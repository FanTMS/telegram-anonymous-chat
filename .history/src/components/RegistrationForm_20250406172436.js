...
import { troubleshootCollection, ensureRequiredCollectionsExist } from '../utils/firebaseUtils';
import useTelegram from '../utils/useTelegram'; // Исправленный импорт
...
const RegistrationForm = ({ onSubmit, telegramUser = null, isDevelopment = false }) => {
    ...
    const enhancedHandleSubmit = useCallback(async () => {
        try {
            ...
            // Проверка и создание необходимых коллекций перед отправкой формы
            try {
                ...
                // Дополнительная диагностика коллекции users
            ...
            const registeredUserData = await onSubmit(userData);

            // Обновляем состояние после успешной регистрации
            if (registeredUserData) {
                setRegisteredUser(registeredUserData);
                setRegistrationComplete(true);
                // Сохраняем в локальном хранилище для случаев обновления страницы
                localStorage.setItem('registeredUser', JSON.stringify(registeredUserData));
            }
            ...
            setErrors((prev) => ({
                ...prev,
                submit: `Ошибка регистрации: ${error.message || 'Неизвестная ошибка'}`
            }));
    ...
    }, [currentStep, formData, onSubmit, validateStep, setRegisteredUser, safeHapticFeedback, safeShowPopup]);
