...
const GroupMessages = ({ groupId, user, isMember }) => {
    ...
    const handleSendMessage = async (e) => {
        ...
        const messageText = message.trim(); // Сохраняем текст сообщения в переменную
        setIsSending(true);
        setMessage(''); // Очищаем поле ввода

        try {
            await sendGroupMessage(groupId, user.id, messageText);
            
            // Обновляем список сообщений локально для более быстрого отображения
            const newMessageObj = {
                id: 'temp-' + Date.now(),
                text: messageText,
                senderId: user.id,
