...
export const BeginnerGuide: React.FC = () => {
  ...
  // Сохраняем факт посещения руководства в Firebase
  const saveVisit = async () => {
    try {
      const userId = localStorage.getItem('current_user_id');
      if (userId) {
        await firebase.setItem('user_activity', `${userId}_guide_visit`, {
          timestamp: Date.now().toString(), // Преобразуем число в строку
          userId
        });
      }
    } catch (error) {
      console.error("Error saving guide visit:", error);
    }
  };

  // Инициализация при загрузке компонента
  useEffect(() => {
    // Получаем случайный совет - эта функция не требует Firebase
    const tip = getRandomTip();
    setRandomTip(tip);

    // Получаем категории шаблонов - эта функция не требует Firebase
  ...
  const handleTabClick = (tab: 'tips' | 'templates' | 'examples') => {
    setActiveTab(tab);
    if (typeof WebApp !== 'undefined' && WebApp.HapticFeedback) {
      WebApp.HapticFeedback.selectionChanged();
    }

    // Сохраняем выбор раздела в Firebase
    const userId = localStorage.getItem('current_user_id');
    if (userId) {
      firebase.setItem('user_activity', `${userId}_guide_view`, {
        timestamp: Date.now().toString(), // Преобразуем число в строку
        section: activeTab
      });
    }
