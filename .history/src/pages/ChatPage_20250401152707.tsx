...
const ChatPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(chatRef, async (docSnap) => {
            if (docSnap.exists()) {
                ...
                if (chatData.unreadMessages && chatData.unreadMessages[auth.currentUser.uid] > 0) {
                    const updateData: any = { unreadMessages: { ...chatData.unreadMessages } };
                    updateData.unreadMessages[auth.currentUser.uid] = 0;
    ...
    const sendMessage = async () => {
        ...
        const otherUser = chatData.participants.find((id: string) => id !== auth.currentUser?.uid);
