                ...
                {groupedMessages.length === 0 ? (
                    ...
                    groupedMessages.map((item, index) => (
                        item.type === 'date' ? (
                            ...
                            <div
                                key={`msg-${index}`}
                                className={`message ${item.value.senderId === user.telegramId ? 'my-message' : 'partner-message'} ${item.value.isFromSupport ? 'support-message' : ''}`}
                            >
                                <div className="message-bubble">
                                    {item.value.isFromSupport && (
                                        <div className="message-sender">Техническая поддержка</div>
                                    )}
