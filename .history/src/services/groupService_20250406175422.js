...
export const sendGroupMessage = async (groupId, messageData) => {
    try {
        if (!groupId) {
            console.warn('sendGroupMessage: groupId не определен');
            throw new Error('Идентификатор группы не указан');
        }

        if (!messageData) {
            console.warn('sendGroupMessage: messageData не определен');
            throw new Error('Данные сообщения не указаны');
        }
        ...
        // Добавляем временную метку, если она не указана
        ...
        const messageRef = await addDoc(collection(db, 'groupMessages'), messageWithTimestamp);
        
        // Обновляем lastActivity группы
        await updateDoc(doc(db, 'groups', groupId), {
            lastActivity: serverTimestamp(),
            lastMessage: messageWithTimestamp.text.substring(0, 50) // Сохраняем первые 50 символов сообщения
        });
 ...
 */
 * @param {Object} groupData - Данные для обновленияt = 50) => {
...
export const updateGroup = async (groupId, groupData, userId) => {
    try {
        // Проверяем существование группы
        const groupRef = doc(db, "groups", groupId);сти сложного индекса
        const groupDoc = await safeFirestoreQuery(() => getDoc(groupRef));
        const messagesQuery = query(
        if (!groupDoc.exists()) {Messages"),
            throw new Error(`Группа с ID ${groupId} не найдена`);
        }   where("isDeleted", "==", false),
            // Удаляем сортировку, чтобы не требовался индекс
        const currentGroupData = groupDoc.data();

        // Проверяем права пользователя (только админ может изменять группу)sQuery));
        ...
        // Проверяем, какие поля можно обновитьоне клиента
        ...
        allowedFields.forEach(field => {
            ...
            }   // Обрабатываем случаи, когда поле createdAt может быть timestamp, date или отсутствовать
        });     const timeA = a.createdAt?.toMillis ? a.createdAt.toMillis() :
                    a.createdAt?.getTime ? a.createdAt.getTime() : 0;
        // Если нет данных для обновления?.toMillis ? b.createdAt.toMillis() :
        if (Object.keys(updateData).length === 0) {dAt.getTime() : 0;
            return true;imeB - timeA; // Сортировка от новых к старым
        }   })
            .slice(0, limit); // Ограничиваем количество сообщений на стороне клиента
        // Обновляем время последней активности
        updateData.lastActivity = new Date().toISOString();
    } catch (error) {
        // Применяем обновленияри получении сообщений группы ${groupId}:`, error);
        await safeFirestoreQuery(() => updateDoc(groupRef, updateData));и
    }
        return true;
    } catch (error) {
        console.error(`Ошибка при обновлении группы ${groupId}:`, error);
        throw error;цию о группе
    }aram {string} groupId - ID группы
}; @param {Object} groupData - Данные для обновления
 * @param {string} userId - ID пользователя, выполняющего обновление
/**@returns {Promise<boolean>} true в случае успеха
 * Поиск групп по названию или тегам
 * @param {string} query - Поисковый запросgroupData, userId) => {
 * @param {number} limitCount - Ограничение количества результатов
 * @returns {Promise<Array<Group>>} Массив объектов групп
 */     const groupRef = doc(db, "groups", groupId);
export const searchGroups = async (query, limitCount = 20) => {groupRef));
    try {
        const searchQuery = query.toLowerCase().trim();
            throw new Error(`Группа с ID ${groupId} не найдена`);
        // Поиск возможен только среди публичных групп
        const groupsQuery = query(
            collection(db, "groups"),pDoc.data();
            where("isPublic", "==", true),
            orderBy("name"),ользователя (только админ может изменять группу)
            limit(limitCount).members ||
        );  !currentGroupData.members[userId] ||
            currentGroupData.members[userId].role !== 'admin') {
        const groupsSnapshot = await safeFirestoreQuery(() => getDocs(groupsQuery));
        }
        // Фильтруем результаты на клиенте
        return groupsSnapshot.docsожно обновить
            .map(doc => Group.fromFirestore(doc))on', 'isPublic', 'isAnonymous', 'avatar', 'tags'];
            .filter(group => {
    ...
    } catch (error) {ys(updateData).length === 0) {
