...
const createTestDocumentsForIndexes = async () => {
    try {
        ...
        // Создаем тестовые документы для индекса messages
        await setDoc(doc(db, 'messages', 'test_message_index_1'), {
            chatId: 'test_chat_1',
            timestamp: new Date(Date.now() - 1000),
            text: 'Тестовое сообщение для создания индекса'
        });
        
        await setDoc(doc(db, 'messages', 'test_message_index_2'), {
            chatId: 'test_chat_1',
            timestamp: new Date(),
            text: 'Тестовое сообщение для создания индекса #2'
        });
