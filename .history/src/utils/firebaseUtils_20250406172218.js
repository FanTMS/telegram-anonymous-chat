...
export const ensureRequiredCollectionsExist = async () => {
    ...
    try {
        ...
        // Специальная обработка ошибки BloomFilter
        if (error?.name === 'BloomFilterError') {
            console.warn('Обнаружена ошибка BloomFilter, это не влияет на основные функции', error);
...
/**
 * Безопасный запрос к коллекции с обработкой ошибок BloomFilter
 * @param {function} queryFunction - Функция для выполнения запроса
 * @returns {Promise<any>} - Результат запроса
 */
export const safeFirestoreQuery = async (queryFunction) => {
    try {
        return await queryFunction();
    } catch (error) {
        // Обработка ошибки BloomFilter
        if (error?.name === 'BloomFilterError') {
            console.warn('Игнорирование ошибки BloomFilter при запросе', error);

            // Пытаемся выполнить запрос еще раз без BloomFilter
            // Это возможное решение, но может не работать для всех случаев
            try {
                return await queryFunction();
            } catch (retryError) {
