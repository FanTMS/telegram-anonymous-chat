    ...
    try {
        ...
        if (!userId) {
            try {
                ...
                console.warn('Не удалось получить данные пользователя из storage:', error);
            }
        }
        ...
        await addSupportChat(userId, message);
...
/**
 * Добавление запроса пользователя в чат с поддержкой
 * @param {string} userId - ID пользователя
...
export const addSupportChat = async (userId, message) => {
    try {
        ...
        if (!supportChatId) {
            // Создаем новый чат с поддержкой
