...
const GroupDetail = ({ groupId, user }) => {
    ...
    useEffect(() => {
        const loadGroup = async () => {
            try {
                ...
                // Пользователь является активным участником группы только если у него isActive: true
                setIsMember(!!userMembership && userMembership.isActive === true);
                setIsAdmin(userMembership?.role === 'admin' && userMembership?.isActive === true);
            } catch (err) {
        ...
        if (groupId) {
            ...
            // Периодическое обновление данных группы каждые 15 секунд
            const intervalId = setInterval(loadGroup, 15000);
    ...
    // Обновляем только статус участия в группе без автоматического присоединения
    useEffect(() => {
        const checkMembership = async () => {
            if (!group || !user?.id) return;

            try {
                // Проверяем, является ли пользователь активным членом группы
                const userMembership = group.members?.[user?.id];
                const isMemberActive = !!userMembership && userMembership.isActive === true;
                
                // Обновляем статус участника
                setIsMember(isMemberActive);
            } catch (error) {
    ...
    const handleJoinGroup = async () => {
        if (!user?.id) {
            ...
            try {
                await safeShowPopup({
                    ...
                    message: 'Войдите в систему, чтобы присоединиться к группе',
                    buttons: [{ text: "OK" }]
                });
            } catch (error) {
        ...
        try {
            ...
            try {
                await safeShowPopup({
                    title: 'Ошибка',
                    message: 'Не удалось присоединиться к группе: ' + err.message,
                    buttons: [{ text: "OK" }]
                });
            } catch (popupError) {
    ...
    const handleLeaveGroup = async () => {
        ...
        try {
            setProcessing(true);
            console.log('Attempting to leave group:', groupId, 'User ID:', user.id);
            ...
            // Перенаправляем пользователя на список групп
            navigate('/groups');
        } catch (err) {
            console.error('Ошибка при выходе из группы:', err);
            alert('Не удалось покинуть группу: ' + (err.message || 'Неизвестная ошибка'));
        } finally {
            setProcessing(false);
        }
    };
    ...
    // Подсчитываем активных участников группы
    const getActiveMemberCount = () => {
        if (!group || !group.members) return 0;
        
        // Если есть поле memberCount, используем его как базовое значение
        if (group.memberCount !== undefined) {
            return Math.max(0, group.memberCount);
        }
        
        // Иначе считаем вручную количество активных участников
            ...
            <GroupHeader>
                ...
                <GroupInfo>
                    ...
                    <GroupMeta>
                        ...
                        <GroupTypeLabel>
                            {activeMemberCount} {getWordForm(activeMemberCount, ['участник', 'участника', 'участников'])}
                        </GroupTypeLabel>
                    </GroupMeta>
