...
export const Chat = () => {
  ...
  const loadChatData = async () => {
    ...
    try {
      ...
      // Если ID чата передан через параметры - используем его, иначе берем из Firebase
      ...
      const chatData = await getChatById(targetChatId);
      ...
      const gameRequest = await getGameRequestForChat(targetChatId);
      const game = await getActiveGameForChat(targetChatId);
      ...
      await markChatNotificationAsRead(currentUser.id);
  ...
  useEffect(() => {
    const loadChatData = async () => {
      try {
        // Пропускаем загрузку, если чат уже загружен
        if (chat) return;
        ...
        const gameResult = await getActiveGameForChat(chatId || '');
        // Проверяем что gameResult не Promise
        if (gameResult && typeof gameResult === 'object') {
          setActiveGame(gameResult);
      ...
      if (chatId || localStorage.getItem('active_chat_id')) {
        ...
        try {
          ...
          if (updatedChat) {
            ...
            const gameRequest = await getGameRequestForChat(id || '');
            const game = await getActiveGameForChat(id || '');
  ...
  useEffect(() => {
    const loadChat = async () => {
      try {
        ...
        // Получаем данные об игре
        const game = await getActiveGameForChat(chatId || '');
        if (game) {
          setActiveGame(game);
          if (!game.isCompleted) {
            setShowGameInterface(true);
  ...
  useEffect(() => {
    try {
      if (WebApp && WebApp.isExpanded) {
        ...
        WebApp.BackButton.onClick(() => navigate(-1 as unknown as string));
    ...
    return () => {
      try {
        if (WebApp && WebApp.isExpanded) {
          WebApp.BackButton.offClick(() => navigate(-1 as unknown as string));
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const sentMessage = await sendMessage(chat.id, currentUser.id, currentMessage);
  ...
  const handleEndChat = async () => {
    ...
    try {
      ...
      WebApp.showConfirm(
        ...
        async (confirmed) => {
          if (confirmed) {
            const success = await endChat(chat.id);
            if (success) {
  ...
  const handleAddFriend = async () => {
    ...
    try {
      ...
      const systemMessage = await addSystemMessage(chat.id, `${currentUser.name} отправил запрос в друзья.`);
  ...
  if (!chat) {
    return (
      <Card className="p-4 m-2">
        ...
        <Button onClick={() => navigate(-1 as unknown as string)} className="w-full">Вернуться назад</Button>
      </Card>
    );
  }
