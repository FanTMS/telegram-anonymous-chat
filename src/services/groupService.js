...
export const joinGroup = async (groupId, userId) => {
    try {
        ...
        if (groupData.members && groupData.members[userId]) {
            // Проверяем, активен ли пользователь в группе
            if (groupData.members[userId].isActive === true) {
                console.warn(`Пользователь ${userId} уже является активным членом группы ${groupId}`);
                return true;
            } else {
                // Если пользователь был в группе, но вышел (isActive === false), 
                // то активируем его членство заново
                console.log(`Пользователь ${userId} повторно присоединяется к группе ${groupId}`);
                
                await safeFirestoreQuery(() =>
                    updateDoc(groupRef, {
                        [`members.${userId}.isActive`]: true,
                        [`members.${userId}.joinedAt`]: timestamp,
                        [`members.${userId}.leftAt`]: null,
                ...
                // Добавляем системное сообщение о повторном присоединении пользователя
                await safeFirestoreQuery(() =>
                    addDoc(collection(db, "groupMessages"), {
                        groupId: groupId,
                        senderId: "system",
                        senderName: "Система",
                        text: `Пользователь вернулся в группу`,
                        createdAt: timestamp,
                        type: "system",
                        isDeleted: false,
    ...
    try {
        ...
        if (groupData.members[userId].role === 'admin') {
            const adminCount = Object.entries(groupData.members).filter(
                ([_, member]) => member.role === 'admin' && member.isActive === true
            ).length;

            if (adminCount === 1) {
                // Если пользователь последний администратор, предлагаем удалить группу
                const shouldDelete = window.confirm(
                    'Вы последний администратор группы. Хотите удалить группу полностью?'
                );

                if (shouldDelete) {
                    // Удаляем группу
                    await safeFirestoreQuery(() => deleteDoc(groupRef));
                    ...
                    const deletePromises = messagesSnapshot.docs.map(doc => 
                        safeFirestoreQuery(() => deleteDoc(doc.ref))
                    );
                    
                    await Promise.all(deletePromises);
                    
                    // Удаляем группу из списка групп всех пользователей
                    const usersRef = collection(db, "users");
                    const usersQuery = query(usersRef, where("groups", "array-contains", groupId));
